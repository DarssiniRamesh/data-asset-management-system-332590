...
// PUBLIC_INTERFACE
export function ParentInputParameterMappingTab({ caps }: AssetDetailsTabProps) {
  /** Contract:
   * Output:
   *  - Placeholder panel for BRD-required tab.
   * Notes:
   *  - Wiring to /api/assets/{assetId}/input-parameters/{childInputParameterId}/parent-input-mappings is a later step.
   *  - Backend enforces cycle-prevention; UI will surface those validation errors when wired.
   */
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-950">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="muted mt-1">BRD-required parent mapping module for the selected child input parameter.</div>
        </div>
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700 dark:bg-slate-900/60 dark:text-slate-200">
        Not yet wired in this step. Planned API:{" "}
        <code className="font-mono">
          /api/assets/{`{assetId}`}/input-parameters/{`{childInputParameterId}`}/parent-input-mappings
        </code>
        .
      </div>
    </div>
