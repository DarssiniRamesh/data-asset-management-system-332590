...
// PUBLIC_INTERFACE
export function ReportingAttributesMappingTab({ caps }: AssetDetailsTabProps) {
  /** Contract:
   * Output:
   *  - Placeholder panel for BRD-required tab.
   * Notes:
   *  - Wiring to /api/assets/{assetId}/reporting-attribute-mappings is a later step.
   *  - Backend enforces uniqueness; UI will surface conflicts when wired.
   */
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-950">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold">Reporting Attributes Mapping</div>
          <div className="muted mt-1">BRD-required reporting attributes mapping module.</div>
        </div>
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700 dark:bg-slate-900/60 dark:text-slate-200">
        ...
        <code className="font-mono">/api/assets/{`{assetId}`}/reporting-attribute-mappings</code>.
      </div>
    </div>
  );
}
